...
companyType: "",
  ...
  const handleSaveRestaurant = async () => {
    try {
      ...
      const RestaurantInformationPayload={
        ...
        restaurantType:restaurantFormData.restaurantType,
         cuisineTypes: restaurantFormData.cuisineTypes.split(",") .map((item) => item.trim()).filter(Boolean),
  ...
  phone: restaurantFormData.contactPhone
      ...
      closingTime: restaurantFormData.closingTime
        ...
        const response = await api.put(`/restaurant/update-restaurantinfo`, RestaurantInformationPayload);
...
setEditingRestaurant(false);
  ...
  //Legal Informations
  const [legalInfoData, setlegalInfoData] = useState();
  const [editinglegalInfo, setEditingLegalInfo] = useState(false);
  const [legalInfoFormData, setLeglInfoFormData] = useState({

 legalName:legalInfoData?.legalName||"",
 companyType:legalInfoData?.companyType||""
 })
  ...
  const handleSaveLegalInfo=async()=>{
    try {
      setIsLoading(true)
      const LegalInfoPayload={
legalName:legalInfoData.legalName,
companyType:legalInfoData.companyType
      }
      const response=await api.put(`/restaurant/update-legalinfo`,LegalInfoPayload)
       toast.success("LegalInfo updated successfully");
      await fetchRestaurantData();
      setEditingLegalInfo(false);
    } catch (error) {
      toast.error(
        error.response?.data?.message || "Failed to update LegalInfo",
      );
    }finally {
      setIsLoading(false);
    }
...
const handleCancelLegalInfo=async()=>{
setLeglInfoFormData({
legalName:legalInfoData?.legalName||"",
 companyType:legalInfoData?.companyType||""
})
setEditingLegalInfo(false)
  ...
  const fetchRestaurantData = async () => {
    try {
     ...
     setLegalInfoData(res.data.data.documents);
    } catch (error) {
      toast.error(
